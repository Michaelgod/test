...
def process_clip(src_dir, dst_dir, seq_len, img_size, mean=None, normalization=True,
    ...
    if seq_len is None:
        ...
        dst_dir = os.path.splitext(dst_dir)[0]+'.npy'
#        dst_dir = os.path.join(data_dir, 'UCF-Preprocessed-OF')
...
def calc_mean(UCF_dir, img_size):
    ...
    mean = scipy.misc.imread(mean, img_size)
...
def regenerate_data(data_dir, list_dir, UCF_dir):
    ...
    dest_dir = 'E:/ActionRecognition/data/OF_data'
...
if __name__ == '__main__':
    ...
    txt_dir = os.path.join(list_dir, 'testlist.txt')
    dest_dir = os.path.join(list_dir, 'testlist01.txt')
